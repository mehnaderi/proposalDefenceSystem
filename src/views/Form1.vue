<template>
  <div class="mb-5 pb-5">
    <div class="row container1 mr-0 ml-0">
      <horizontal-stepper :active-step-number="1"></horizontal-stepper>
    </div>
          ...
          <form @click.prevent>
            <div class="row">
              <div class="col-25" dir="rtl">
                <label for="rsType">نام و نام خانوادگی استاد راهنما</label>
              </div>
              <div class="col-75 customSelect">
                <select id="rsType" name="researchType" v-model="selected">
                  ...
                  <option v-for="(master,i) in masters" :key="i" :value="master">{{master}}</option>
                </select>
              </div>
            </div>
          </form>
      ...
      <div class="col-5 mt-5 border-top">
        <div class="row mt-4">
          <div class="col-5 mt-1">
            <!-- <router-link to="/register/0" class="btn btn-default p-3 text-secondary">
              <span class="fas fa-arrow-left" />
              <small class="ml-2">بازگشت</small>
            </router-link>-->
          </div>
          <div class="col-7 d-flex align-items-end flex-column">
            <!-- <router-link to="/register/2" class="btn btn-primary p-3">
              <small>
                بعدی: اطلاعات پایان
                نامه
              </small>
...
export default {
  ...
  methods: {
    ...
    onNext() {
      ...
      this.$store.dispatch("savePreferedProf", formData);
    }
  },
  computed: {
    all_filled() {
      if (this.selected != "انتخاب کنید") {
        return false;
      } else {
        return true;
      }
